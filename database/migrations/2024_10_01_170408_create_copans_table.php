...
class CreateCopansTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('copans', function (Blueprint $table) {
            ...
            $table->string('code');
            $table->string('amount');
            $table->tinyInteger('amount_type')->default(2)->comment('2 => percentage, 1 => price unit');
            $table->unsignedBigInteger('discount_ceiling')->nullable();
            $table->tinyInteger('type')->default(2)->comment('2 => common (means each user can use one time), 1 => private (means one user can use one time)');
